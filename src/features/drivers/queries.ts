...
export async function getDriverDetailStats(driverId: string, params?: { startDate?: Date; endDate?: Date; date?: Date }) {
  ...
  const [driver, ordersGrouped, bottleStats, recentOrders, allTimeStats, todayStats, expenseStats] = await Promise.all([
    ...
    // Group orders by status to get counts and sums for the selected period
    db.order.groupBy({
      by: ['status'],
  ...
  for (const group of ordersGrouped) {
    ...
    if (group.status === OrderStatus.COMPLETED) {
      ...
      totalCashCollected = totalCashCollected.plus(group._sum.cashCollected || 0);
      totalRevenue = totalRevenue.plus(group._sum.totalAmount || 0);
    } else if (([OrderStatus.SCHEDULED, OrderStatus.IN_PROGRESS] as OrderStatus[]).includes(group.status)) {
      pendingOrders += count;
  ...
  // Calculate average locally since aggregate is removed
  const avgCashCollected = completedOrders > 0 ? (parseFloat(totalCashCollected.toString()) / completedOrders).toFixed(2) : '0';
  ...
  return {
    ...
    financial: {
      ...
      averageCashPerDelivery: avgCashCollected,
    },
    bottles: {
