...
export async function getDriverDetailStats(driverId: string, params?: { startDate?: Date; endDate?: Date; date?: Date }) {
  ...
  const [driver, orderStatsByStatus, bottleStats, recentOrders, allTimeStats, todayStats, expenseStats] = await Promise.all([
    // Driver basic info
    ...
    // Order counts and financial stats grouped by status for selected period
    db.order.groupBy({
      by: ['status'],
  ...
  // Calculate order counts from the grouped stats
  let totalOrders = 0;
  let completedOrders = 0;
  let pendingOrders = 0;
  let cancelledOrders = 0;
  let rescheduledOrders = 0;
  let completedCashCollected = new Prisma.Decimal(0);
  let completedTotalAmount = new Prisma.Decimal(0);
  let completedCashAvg = new Prisma.Decimal(0);

  orderStatsByStatus.forEach((stat) => {
    const count = stat._count._all;
    totalOrders += count;

    if (stat.status === OrderStatus.COMPLETED) {
      completedOrders = count;
      completedCashCollected = stat._sum.cashCollected || new Prisma.Decimal(0);
      completedTotalAmount = stat._sum.totalAmount || new Prisma.Decimal(0);
      completedCashAvg = stat._avg.cashCollected || new Prisma.Decimal(0);
    } else if (stat.status === OrderStatus.PENDING || stat.status === OrderStatus.SCHEDULED || stat.status === OrderStatus.IN_PROGRESS) {
      pendingOrders += count;
    } else if (stat.status === OrderStatus.CANCELLED) {
      cancelledOrders = count;
    } else if (stat.status === OrderStatus.RESCHEDULED) {
  ...
  return {
    ...
    financial: {
      totalCashCollected: completedCashCollected.toString(),
      totalRevenue: completedTotalAmount.toString(),
      averageCashPerDelivery: completedCashAvg.toString(),
    },
    bottles: {
