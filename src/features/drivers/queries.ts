...
export async function getDriverDetailStats(driverId: string, params?: { startDate?: Date; endDate?: Date; date?: Date }) {
  ...
  const [driver, orderStatsGrouped, financialStats, bottleStats, recentOrders, allTimeStats, todayStats, expenseStats] = await Promise.all([
    // Driver basic info
  ...
  const pendingStatuses = [OrderStatus.SCHEDULED, OrderStatus.IN_PROGRESS];
