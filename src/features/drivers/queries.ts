...
export async function getDriverDetailStats(driverId: string, params?: { startDate?: Date; endDate?: Date; date?: Date }) {
  ...
  const [driver, ordersByStatus, financialStats, bottleStats, recentOrders, allTimeStats, todayStats, expenseStats] = await Promise.all([
    // Driver basic info
    ...
    // Order counts for selected period (grouped by status)
    db.order.groupBy({
      by: ['status'],
  ...
  const pendingStatuses: OrderStatus[] = [OrderStatus.SCHEDULED, OrderStatus.IN_PROGRESS];
  const pendingOrders = ordersByStatus.filter((g) => pendingStatuses.includes(g.status)).reduce((sum, group) => sum + group._count.id, 0);
