...
export async function getDriverDetailStats(driverId: string, params?: { startDate?: Date; endDate?: Date; date?: Date }) {
  ...
  const [driver, ordersGrouped, bottleStats, recentOrders, allTimeStats, todayStats, expenseStats] = await Promise.all([
    ...
    // Consolidate Order counts and Financial statistics into one query
    db.order.groupBy({
      by: ['status'],
  ...
  ordersGrouped.forEach((group) => {
    ...
    if (group.status === OrderStatus.COMPLETED) {
      ...
      totalCashCollected = totalCashCollected.plus(group._sum.cashCollected || new Prisma.Decimal(0));
      totalRevenue = totalRevenue.plus(group._sum.totalAmount || new Prisma.Decimal(0));
    } else if ([OrderStatus.SCHEDULED, OrderStatus.IN_PROGRESS].includes(group.status as any)) {
      pendingOrders += count;
  ...
  return {
    ...
    financial: {
      ...
      averageCashPerDelivery: averageCashPerDelivery,
    },
    bottles: {
