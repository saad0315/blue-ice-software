...
export async function getComprehensiveDashboardData(params?: { startDate?: Date; endDate?: Date }) {
  ...
  // We derive totalRevenue, totalCompletedOrders, and totalVolume from ordersByStatus after Promise.all
  ...
  const previousRevenueValue = prevRevenueVal;
  const revenueChange = previousRevenueValue > 0 ? ((currentRevenueValue - previousRevenueValue) / previousRevenueValue) * 100 : 0;
  const ordersChange = prevOrdersVal > 0 ? ((totalVolume - prevOrdersVal) / prevOrdersVal) * 100 : 0;
