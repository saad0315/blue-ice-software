...
export async function getComprehensiveDashboardData(params?: { startDate?: Date; endDate?: Date }) {
  ...
  if (!isHistoricalOnly) {
    // Optimize: Run trends and breakdown in parallel to derive totals
    const [liveTrendRaw, statusGroups] = await Promise.all([
