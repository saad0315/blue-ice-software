...
export async function getComprehensiveDashboardData(params?: { startDate?: Date; endDate?: Date }) {
  ...
  // Derive Low Stock Products from productInventory
  const derivedLowStockProducts = productInventory
    .filter((p) => p.stockFilled < 20)
    .sort((a, b) => a.stockFilled - b.stockFilled);

  // Derive Total Cash Collected from ordersByPaymentMethod
  const derivedTotalCashCollected = ordersByPaymentMethod.reduce(
    (sum, p) => sum + parseFloat(p._sum.cashCollected?.toString() || '0'),
    0,
  );

  // Derive Total Revenue, Volume, Completed Orders from ordersByStatus
  const totalVolume = ordersByStatus.reduce((acc, curr) => acc + curr._count.id, 0);
  const completedStats = ordersByStatus.find((s) => s.status === OrderStatus.COMPLETED);
  const totalCompletedOrders = completedStats?._count.id || 0;
  const totalRevenue = parseFloat(completedStats?._sum.totalAmount?.toString() || '0');
  ...
  return {
    ...
    cashManagement: {
      totalCashCollected: derivedTotalCashCollected,
