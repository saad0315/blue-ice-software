...
export async function getComprehensiveDashboardData(params?: { startDate?: Date; endDate?: Date }) {
  ...
  if (!isHistoricalOnly) {
    // Live Revenue Trend (Group by Date) - Optimization: Fetch once and aggregate in memory
    ...
    // Derive Live Aggregates from Trends
    ...
    // Derive Live Total Volume from Status Groups
  ...
  // Derived Metrics (Optimization: In-memory calculation)
