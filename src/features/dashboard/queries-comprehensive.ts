...
export async function getComprehensiveDashboardData(params?: { startDate?: Date; endDate?: Date }) {
  ...
  // Derive cash stats and low stock from existing fetches to avoid redundant DB queries
  const derivedTotalCashCollected = ordersByPaymentMethod.reduce((sum, p) => sum + Number(p._sum.cashCollected || 0), 0);
