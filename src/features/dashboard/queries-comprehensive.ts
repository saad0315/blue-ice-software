...
export async function getComprehensiveDashboardData(params?: { startDate?: Date; endDate?: Date }) {
  ...
  // ⚡ Bolt Optimization: Derive cashStats and lowStockProducts in-memory to save parallel db queries
  const cashStats = {
    _sum: {
      cashCollected: ordersByPaymentMethod.reduce((sum, p) => sum + parseFloat(p._sum.cashCollected?.toString() || '0'), 0),
    },
  };
