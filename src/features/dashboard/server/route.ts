...
const app = new Hono()
  .get('/', sessionMiddleware, async (ctx) => {
    ...
    try {
      ...
      // Performance Optimization: Derive total orders, active orders, and revenue
      // in-memory from the order status distribution query to avoid redundant DB queries
      ...
      let totalRevenueNum = new Prisma.Decimal(0);
      ...
      orderStatusDistribution.forEach((group) => {
        ...
        if (group.status === OrderStatus.COMPLETED) {
          ...
          totalRevenueNum = totalRevenueNum.add(amount);
        }
      });
