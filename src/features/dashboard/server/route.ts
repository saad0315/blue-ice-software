  ...
  .get('/', sessionMiddleware, async (ctx) => {
    ...
    try {
      ...
      const activeOrderCount = orderStatusDistribution
        .filter((curr) => ![OrderStatus.COMPLETED, OrderStatus.CANCELLED].includes(curr.status as OrderStatus))
        .reduce((acc, curr) => acc + curr._count.id, 0);

      const completedStats = orderStatusDistribution.find((curr) => curr.status === OrderStatus.COMPLETED);
      const totalRevenue = new Prisma.Decimal(completedStats?._sum?.totalAmount || 0).toString();
