...
const app = new Hono()
  .get('/', sessionMiddleware, async (ctx) => {
    ...
    try {
      ...
      // Optimize: Calculate aggregated metrics in-memory from the distribution query
      // This saves 3 redundant database queries (count, active count, and revenue aggregate)
      ...
      const activeOrderCount = orderStatusDistribution
        .filter((curr) => curr.status !== OrderStatus.COMPLETED && curr.status !== OrderStatus.CANCELLED)
        .reduce((acc, curr) => acc + curr._count.id, 0);

      const revenueData = orderStatusDistribution.find((curr) => curr.status === OrderStatus.COMPLETED)?._sum.totalAmount || 0;
      ...
      return ctx.json({
        data: {
          ...
          orderCount,
          activeOrderCount,
          totalRevenue: revenueData.toString(),
