...
const app = new Hono()
  .get('/', sessionMiddleware, async (ctx) => {
    ...
    try {
      ...
      // ⚡ Bolt: Derived metrics in-memory from orderStatusDistribution to reduce DB calls (from 6 to 3)
      ...
      const completedStats = orderStatusDistribution.find((item) => item.status === OrderStatus.COMPLETED);
      const totalRevenue = completedStats?._sum?.totalAmount?.toString() || '0';
