...
const app = new Hono()
  .get('/', sessionMiddleware, async (ctx) => {
    ...
    try {
      ...
      // ⚡ Bolt Optimization: Combined 4 queries into 2 to reduce database load.
      // orderCount, activeOrderCount, and revenueData are now derived from orderStatusDistribution.
      ...
      // Calculate derived metrics in-memory
      ...
      const totalRevenue =
        orderStatusDistribution.find((curr) => curr.status === OrderStatus.COMPLETED)?._sum.totalAmount?.toString() || '0';
