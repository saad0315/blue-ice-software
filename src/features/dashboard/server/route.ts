...
const app = new Hono()
  .get('/', sessionMiddleware, async (ctx) => {
    ...
    try {
      ...
      // Performance Optimization: Replaced multiple distinct `.count()` and `.aggregate()` queries
      // with a single `.groupBy()` query, deriving total orders, active orders, and revenue in-memory.
      const [customerCount, dailyRevenue, orderStatusDistribution] = await Promise.all([
        ...
        `,
        // Order Status Distribution and Aggregations
        db.order.groupBy({
          by: ['status'],
          _count: {
            id: true,
      ...
      orderStatusDistribution.forEach((group) => {
        ...
        if (group.status === OrderStatus.COMPLETED && group._sum.totalAmount) {
          totalRevenue = totalRevenue.plus(group._sum.totalAmount);
        }
      });
