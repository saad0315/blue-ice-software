  ...
  .get('/', sessionMiddleware, async (ctx) => {
    ...
    try {
      ...
      orderStatusDistribution.forEach((group) => {
        ...
        if (!([OrderStatus.COMPLETED, OrderStatus.CANCELLED] as OrderStatus[]).includes(group.status)) {
          activeOrderCount += group._count.id;
        }

        if (group.status === OrderStatus.COMPLETED && group._sum?.totalAmount) {
          totalRevenueDecimal = totalRevenueDecimal.plus(group._sum.totalAmount);
        }
      });
