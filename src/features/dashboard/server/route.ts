...
const app = new Hono()
  .get('/', sessionMiddleware, async (ctx) => {
    ...
    try {
      ...
      // ⚡ Bolt: Derived metrics in-memory from groupBy to reduce database queries
      ...
      const completedData = orderStatusDistribution.find((item) => item.status === OrderStatus.COMPLETED);
      const totalRevenue = completedData?._sum?.totalAmount?.toString() || '0';
