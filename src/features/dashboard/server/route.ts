...
const app = new Hono()
  .get('/', sessionMiddleware, async (ctx) => {
    ...
    try {
      ...
      // ⚡ Bolt: Consolidated parallel count() and aggregate() queries into a single groupBy()
      // deriving total orders, active orders, and revenue in-memory to reduce DB overhead
      ...
      const formattedDistribution = orderStatusDistribution.map((item) => {
        ...
        if (item.status === OrderStatus.COMPLETED && item._sum?.totalAmount) {
          totalRevenue = totalRevenue.plus(item._sum.totalAmount);
        }
      ...
      return ctx.json({
        data: {
          ...
          orderStatusDistribution: formattedDistribution,
        },
      });
    } catch (error) {
