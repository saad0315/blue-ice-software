  ...
  .get('/', sessionMiddleware, async (ctx) => {
    ...
    try {
      ...
      const activeOrderCount = orderStatusDistribution
        .filter((curr) => ![OrderStatus.COMPLETED, OrderStatus.CANCELLED].includes(curr.status as any))
        .reduce((acc, curr) => acc + curr._count.id, 0);

      const completedGroup = orderStatusDistribution.find((g) => g.status === OrderStatus.COMPLETED);
      const totalRevenue = completedGroup?._sum.totalAmount?.toString() || '0';
