...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [orderGroups, paymentMethodGroups, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Group by order status to replace 5 count queries
    db.order.groupBy({
      by: ['status'],
      where: {
        driverId,
    ...
    // Group by payment method to replace 4 aggregate queries
    db.order.groupBy({
      by: ['paymentMethod'],
  ...
  // Aggregate order counts from grouped data
  let totalOrders = 0;
  ...
  for (const group of orderGroups) {
    ...
    else if ([OrderStatus.PENDING, OrderStatus.SCHEDULED, OrderStatus.IN_PROGRESS].includes(group.status as any)) pendingOrders += count;
  }
  ...
  for (const group of paymentMethodGroups) {
