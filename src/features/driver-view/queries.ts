...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatus, ordersByPaymentMethod, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // ⚡ Bolt Optimization: Combine 5 order count queries into a single groupBy query
    db.order.groupBy({
      by: ['status'],
    ...
    // ⚡ Bolt Optimization: Combine 4 financial aggregate queries into a single groupBy query
    db.order.groupBy({
      by: ['paymentMethod'],
