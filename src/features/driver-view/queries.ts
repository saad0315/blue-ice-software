...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [statusGroups, paymentGroups, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Order counts grouped by status (consolidation of multiple count queries)
    db.order.groupBy({
      by: ['status'],
    ...
    // Financial breakdown by payment method for completed orders (consolidation of multiple aggregate queries)
    db.order.groupBy({
      by: ['paymentMethod'],
  ...
  // Derive status counts in-memory
  let totalOrders = 0;
  ...
  // Derive payment specific metrics in-memory
