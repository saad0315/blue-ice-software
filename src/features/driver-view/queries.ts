...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatus, ordersByPaymentMethod, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    ...
    // Group completed orders by payment method to get financial breakdown in a single query
    db.order.groupBy({
      by: ['paymentMethod'],
  ...
  // Extract counts from grouped status data
  const totalOrders = ordersByStatus.reduce((sum, group) => sum + group._count._all, 0);
  ...
  // Extract financial data from grouped payment method data
  ...
  return {
    ...
    onlineOrdersCount: onlineOrders?._count?._all || 0,
    creditOrdersCount: creditOrders?._count?._all || 0,
    prepaidOrdersCount: prepaidOrders?._count?._all || 0,
