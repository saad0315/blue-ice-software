...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatus, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Grouped order statistics to replace 9 parallel queries
    // ⚡ Bolt Optimization: Group by status and paymentMethod to compute in memory
    db.order.groupBy({
  ...
  // Compute order counts and financial breakdown from ordersByStatus
  let totalOrders = 0;
