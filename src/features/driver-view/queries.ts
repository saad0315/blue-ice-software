...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  // Derive order counts in-memory from grouped data
  ...
  // Derive payment metrics in-memory from grouped data
  const cashGroup = ordersByPaymentMethod.find((g) => g.paymentMethod === PaymentMethod.CASH);
  const onlineGroup = ordersByPaymentMethod.find((g) => g.paymentMethod === PaymentMethod.ONLINE_TRANSFER);
  const creditGroup = ordersByPaymentMethod.find((g) => g.paymentMethod === PaymentMethod.CREDIT);
  const prepaidGroup = ordersByPaymentMethod.find((g) => g.paymentMethod === PaymentMethod.PREPAID_WALLET);
