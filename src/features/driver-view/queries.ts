...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatus, ordersByPaymentMethod, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Group orders by status to avoid multiple count queries
    db.order.groupBy({
      by: ['status'],
    ...
    // Group financial breakdown by payment method to avoid multiple aggregate queries
    db.order.groupBy({
      by: ['paymentMethod'],
  ...
  // Extract payment method aggregations
