...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatusPayment, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Grouped Order counts and financial breakdown
    db.order.groupBy({
  ...
  for (const group of ordersByStatusPayment) {
    const count = group._count.id;
    totalOrders += count;

    if (group.status === OrderStatus.COMPLETED) {
      completedOrders += count;
      const cashSum = parseFloat(group._sum.cashCollected?.toString() || '0');
