...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatus, completedOrdersByPayment, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
  ...
  // Derive order counts from grouped stats
  ...
  // Extract payment method aggregates
  const cashStats = completedOrdersByPayment.find((p) => p.paymentMethod === PaymentMethod.CASH) || {
    _sum: { cashCollected: null },
    _count: { id: 0 },
  };
  const onlineStats = completedOrdersByPayment.find((p) => p.paymentMethod === PaymentMethod.ONLINE_TRANSFER) || {
    _sum: { cashCollected: null },
    _count: { id: 0 },
  };
  const creditStats = completedOrdersByPayment.find((p) => p.paymentMethod === PaymentMethod.CREDIT) || {
    _sum: { cashCollected: null },
    _count: { id: 0 },
  };
  const prepaidStats = completedOrdersByPayment.find((p) => p.paymentMethod === PaymentMethod.PREPAID_WALLET) || {
    _sum: { cashCollected: null },
    _count: { id: 0 },
  };
  ...
  return {
    ...
    cashOrdersCount: cashStats._count.id || 0,
    onlineOrdersCount: onlineStats._count.id || 0,
    creditOrdersCount: creditStats._count.id || 0,
    prepaidOrdersCount: prepaidStats._count.id || 0,
