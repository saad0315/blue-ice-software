...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatus, ordersByPaymentMethod, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Consolidate Order counts by status
    db.order.groupBy({
      by: ['status'],
    ...
    // Consolidate Financial breakdown by payment method
    db.order.groupBy({
      by: ['paymentMethod'],
  ...
  // Derive order counts from ordersByStatus
  ...
  const pendingOrders = ordersByStatus
    .filter((s) => ([OrderStatus.PENDING, OrderStatus.SCHEDULED, OrderStatus.IN_PROGRESS] as string[]).includes(s.status))
  ...
  // Derive financial breakdown from ordersByPaymentMethod
