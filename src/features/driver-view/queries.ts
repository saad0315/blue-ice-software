...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  // ⚡ Bolt: Consolidated 9 separate count/aggregate order queries into a single groupBy query
  const [ordersByGroup, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Group by status and payment method to derive counts and sums in-memory
    db.order.groupBy({
  ...
  // ⚡ Bolt: Derive metrics in-memory from the grouped result
  let totalOrders = 0;
  ...
  for (const group of ordersByGroup) {
    ...
    if (group.status === OrderStatus.COMPLETED) {
      ...
      if (group.paymentMethod === PaymentMethod.CASH) {
        ...
        if (group._sum.cashCollected) cashCollectedSum = cashCollectedSum.plus(group._sum.cashCollected);
      } else if (group.paymentMethod === PaymentMethod.ONLINE_TRANSFER) {
        onlineOrdersCount += count;
        if (group._sum.cashCollected) onlineCollectedSum = onlineCollectedSum.plus(group._sum.cashCollected);
      } else if (group.paymentMethod === PaymentMethod.CREDIT) {
        creditOrdersCount += count;
        if (group._sum.cashCollected) creditGivenSum = creditGivenSum.plus(group._sum.cashCollected);
      } else if (group.paymentMethod === PaymentMethod.PREPAID_WALLET) {
        prepaidOrdersCount += count;
        if (group._sum.cashCollected) prepaidUsedSum = prepaidUsedSum.plus(group._sum.cashCollected);
      }
    } else if (([OrderStatus.PENDING, OrderStatus.SCHEDULED, OrderStatus.IN_PROGRESS] as any[]).includes(group.status)) {
      pendingOrders += count;
