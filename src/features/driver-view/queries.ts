...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [orderStatsGrouped, paymentStatsGrouped, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Group orders by status (replaces 5 separate .count() queries)
    db.order.groupBy({
      by: ['status'],
    ...
    // Group completed orders by payment method (replaces 4 separate .aggregate() queries)
    db.order.groupBy({
      by: ['paymentMethod'],
  ...
  // Derive order status counts in-memory
  let totalOrders = 0;
  ...
  orderStatsGrouped.forEach((group) => {
    const count = group._count.id;
    totalOrders += count;

    if (group.status === OrderStatus.COMPLETED) {
      completedOrders += count;
    } else if (pendingStatuses.includes(group.status as any)) {
      pendingOrders += count;
  ...
  paymentStatsGrouped.forEach((group) => {
