...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatus, ordersByPaymentMethod, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Order counts consolidated via groupBy
    db.order.groupBy({
      by: ['status'],
    ...
    // Financial breakdown by payment method consolidated via groupBy
    db.order.groupBy({
      by: ['paymentMethod'],
  ...
  for (const group of ordersByStatus) {
    ...
    if (group.status === OrderStatus.COMPLETED) {
      completedOrders += group._count._all;
    } else if (group.status === OrderStatus.PENDING || group.status === OrderStatus.SCHEDULED || group.status === OrderStatus.IN_PROGRESS) {
      pendingOrders += group._count._all;
    } else if (group.status === OrderStatus.CANCELLED) {
      cancelledOrders += group._count._all;
    } else if (group.status === OrderStatus.RESCHEDULED) {
      rescheduledOrders += group._count._all;
    }
  }
