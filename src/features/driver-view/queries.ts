...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatus, ordersByPaymentMethod, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Order counts consolidated into one groupBy
    db.order.groupBy({
      by: ['status'],
    ...
    // Financial breakdown by payment method consolidated into one groupBy
    db.order.groupBy({
      by: ['paymentMethod'],
  ...
  const completedOrders = ordersByStatus.find((o) => o.status === OrderStatus.COMPLETED)?._count._all || 0;
  const cancelledOrders = ordersByStatus.find((o) => o.status === OrderStatus.CANCELLED)?._count._all || 0;
  const rescheduledOrders = ordersByStatus.find((o) => o.status === OrderStatus.RESCHEDULED)?._count._all || 0;
  const pendingOrders = ordersByStatus
    .filter((o) => [OrderStatus.PENDING, OrderStatus.SCHEDULED, OrderStatus.IN_PROGRESS].includes(o.status as OrderStatus))
    .reduce((acc, curr) => acc + curr._count._all, 0);

  // Derive payment methods in-memory
  const cashOrders = ordersByPaymentMethod.find((o) => o.paymentMethod === PaymentMethod.CASH);
  const onlineOrders = ordersByPaymentMethod.find((o) => o.paymentMethod === PaymentMethod.ONLINE_TRANSFER);
  const creditOrders = ordersByPaymentMethod.find((o) => o.paymentMethod === PaymentMethod.CREDIT);
  const prepaidOrders = ordersByPaymentMethod.find((o) => o.paymentMethod === PaymentMethod.PREPAID_WALLET);
