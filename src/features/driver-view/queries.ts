...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  // Parse order counts from grouped data
  ...
  const pendingOrders = ordersByStatus
    .filter((g) => pendingStatuses.includes(g.status as OrderStatus))
  ...
  // Extract financial data by payment method
  ...
  return {
    ...
    cashOrdersCount: cashGroup?._count?._all || 0,
    onlineOrdersCount: onlineGroup?._count?._all || 0,
    creditOrdersCount: creditGroup?._count?._all || 0,
    prepaidOrdersCount: prepaidGroup?._count?._all || 0,
