...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatus, ordersByPaymentMethod, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Replace 5 parallel counts with a single groupBy status
    db.order.groupBy({
      by: ['status'],
    ...
    // Replace 4 parallel aggregates with a single groupBy paymentMethod
    db.order.groupBy({
      by: ['paymentMethod'],
  ...
  // Derive financial breakdown and counts in-memory from ordersByPaymentMethod
  let cashCollected = 0;
  ...
  ordersByPaymentMethod.forEach((group) => {
