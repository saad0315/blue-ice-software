...
export async function getDriverStats(driverId: string, date: Date) {
  ...
  const [ordersByStatus, ordersByPaymentMethod, expenseData, bottleData, unlinkedOrdersData, unlinkedExpensesData] = await Promise.all([
    // Group order counts by status (Replaces 5 separate count queries)
    db.order.groupBy({
      by: ['status'],
    ...
    // Financial breakdown by payment method for completed orders (Replaces 4 separate aggregate queries)
    db.order.groupBy({
      by: ['paymentMethod'],
